...
var jobVal='jobTitle_'+(+new Date() * 1e6 + Math.floor(Math.random() * 1e6)).toString(36);
const data =  {
  isComputed: false,
  name: jobVal,
  type: 'int',
...
const options = {
	name: 'JobTitleSelector',
	attributes: {
		placeholder: {value: getLang(placeholder, 'jobTitleSelector'), binding: ''},
		required: false,
		readOnly: false,
		multiSelect: false,
		width: '',
...
setOptions('jobtitleselector', {options, propOptions});
